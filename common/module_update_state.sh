#!/system/bin/sh
# 模块更新状态迁移：仅同架构继承当前字体负载；架构变化必须隔离旧负载后显式重建。
...
# A payload generated by another schema is executable boot input, not a cache.
# Never carry it across an engine boundary: root managers may mount OEM partition
# trees before post-fs-data gets a chance to validate them, which can bootloop.
luoshu_clear_incompatible_payload() {
    _module="$1"
    rm -rf "$_module/system/fonts" 2>/dev/null || true
    mkdir -p "$_module/system/fonts" 2>/dev/null || return 1
    rm -f "$_module/system/etc/fonts.xml" \
          "$_module/system/etc/font_fallback.xml" \
          "$_module/config/font-payload-schema.conf" \
          "$_module/config/font-payload-manifest.conf" \
          "$_module/config/font-payload-boot.conf" \
          "$_module/.font-payload-commit.ok" 2>/dev/null || true
    for _partition in $(luoshu_update_payload_partitions); do
        [ "$_partition" != system ] || continue
        rm -rf "$_module/$_partition" 2>/dev/null || true
    done
    rm -rf "$_module/config/device-font-cache" \
           "$_module/.font-payload-stage."* \
           "$_module/.font-payload-backup."* 2>/dev/null || true
    return 0
}
...
luoshu_migrate_update_cache() {
    ...
    # Config contains persistent immutable artifacts as directories. Metric/source caches are
    # content-addressed and safe across releases. A device payload cache is retained only when
    # its payload schema is unchanged.
...
luoshu_migrate_active_install() {
    ...
    [ -n "$_old_schema" ] && [ "$_old_schema" = "$LUOSHU_PAYLOAD_SCHEMA_CURRENT" ] && _schema_compatible=true

    LUOSHU_UPDATE_ACTIVE="$_active"
    LUOSHU_UPDATE_OLD_SCHEMA="$_old_schema"
    LUOSHU_UPDATE_REBUILD_REQUIRED=false
    LUOSHU_UPDATE_PAYLOAD_PRESERVED=false
    [ "$_active" = default ] || [ "$_schema_compatible" = true ] || LUOSHU_UPDATE_REBUILD_REQUIRED=true
    ...
    # Only payloads produced by the exact same schema may survive an update. On mismatch,
    # retain the user's selected font/config and require the installer to rebuild it before activation.
    if [ "$_active" != default ] && [ "$_schema_compatible" = true ]; then
        ...
        luoshu_clear_incompatible_payload "$_new" || return 1
    fi

    luoshu_migrate_update_cache "$_old" "$_new" "$_schema_compatible"
    luoshu_clear_update_volatile "$_new"

    # active_font.conf records the user's selected font. Effective boot state is tracked separately;
    # a safe-stock migration must not relabel the user's choice as "default" in the App.
