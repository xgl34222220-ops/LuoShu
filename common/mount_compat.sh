...
#
# Compatibility must be non-invasive. Mountify, Magic Mount / Magic Mount RC,
# Hybrid Mount and native module engines consume /data/adb/modules/LuoShu
# directly, so LuoShu must not rewrite their configuration, enforce their
# whitelists, clear their markers, or make a font transaction depend on a
# synthetic probe. Only a real dual-directory metamodule is synchronized.
set +e
...
luoshu_detect_root_manager() {
    if [ -n "${APATCH:-}" ]; then
        ...
        printf 'APatch\n'
    elif [ -x /data/adb/ksu/bin/ksud ] || [ -x /data/adb/ksud ]; then
        printf 'KernelSU\n'
...
_luoshu_magic_mount_present() {
    for _lmmp_path in \
        ...
        _luoshu_module_metadata_matches "$_lmmp_path" \
            magic_mount magic-mount 'magic mount' magic_mount_rc magic-mount-rc \
            'magic mount rc' magic_mount_rs magic-mount-rs id=meta-mm && return 0
    done
...
luoshu_mount_backend() {
    ...
    case "$_lmb_engine" in
        ...
        mountify) printf 'mountify\n' ;;
        magic-mount|magic-mount-rs|magic-mount-rc) printf 'magic-mount\n' ;;
        *) printf 'native\n' ;;
    esac
}

luoshu_direct_source_engine() {
...
# Kept as compatibility APIs for callers from earlier releases. They are
# intentionally advisory/no-op for direct-source engines.
luoshu_mountify_value() { return 1; }
luoshu_mountify_module_selected() { return 0; }
luoshu_magic_mount_ensure_partitions() { return 0; }
luoshu_recover_explicit_disable() { return 0; }
luoshu_recover_magic_mount_markers() { return 0; }

luoshu_mount_preflight() {
    _lmp_engine="${2:-$(luoshu_detect_mount_engine)}"
    LUOSHU_MOUNT_PREFLIGHT_ERROR=''
    luoshu_direct_source_engine "$_lmp_engine" && return 0
...
luoshu_write_mount_probes() {
    ...
    _lwmp_engine="${2:-$(luoshu_detect_mount_engine)}"
    luoshu_direct_source_engine "$_lwmp_engine" && {
        rm -f \
            "$LUOSHU_MOUNT_MODDIR/config/mount-probes-expected.conf" \
    ...
    for _lwmp_partition in $(luoshu_used_partitions); do
        luoshu_meta_partition_supported "$_lwmp_partition" || continue
        ...
        mv -f "$_lwmp_directory/mount-probe.conf.tmp.$$" \
    ...
    [ "$_lwmp_count" -gt 0 ] || {
        rm -f "$_lwmp_temp" 2>/dev/null || true
        return 1
    }
...
luoshu_sync_mount_payload() {
    ...
    # Direct-source metamodules already worked before the compatibility layer.
    # Stay completely out of their way and never turn diagnostics into a font rollback.
    if luoshu_direct_source_engine "$_lsmp_engine"; then
        rm -f \
            "$LUOSHU_MOUNT_MODDIR/config/mount-probes-expected.conf" \
            "$LUOSHU_MOUNT_MODDIR/config/mount-probe-expected.conf" 2>/dev/null || true
        luoshu_mount_record prepared \
            '直读挂载引擎已旁路兼容层：保留标准模块目录，不改配置、不查白名单、不清理标记' \
            '' 0 0 "$_lsmp_partitions"
        luoshu_mount_log "engine=$_lsmp_engine direct-source bypass partitions=$_lsmp_partitions"
        luoshu_mount_lock_release
        trap - EXIT HUP INT TERM
        return 0
    fi

    if ! luoshu_mount_preflight "$_lsmp_active" "$_lsmp_engine"; then
        luoshu_mount_record failed "$LUOSHU_MOUNT_PREFLIGHT_ERROR" '' 0 1 "$_lsmp_partitions"
        luoshu_mount_lock_release
        trap - EXIT HUP INT TERM
        return 1
    fi
    if ! luoshu_write_mount_probes "$_lsmp_active" "$_lsmp_engine"; then
        luoshu_mount_record failed '无法生成双目录元模块挂载探针' '' 0 1 "$_lsmp_partitions"
        luoshu_mount_lock_release
        trap - EXIT HUP INT TERM
        return 1
    fi
    ...
    if [ "$_lsmp_failed" -eq 0 ]; then
        for _lsmp_partition in $(luoshu_used_partitions "$_lsmp_root"); do
            if ! luoshu_meta_partition_supported "$_lsmp_partition"; then
                _lsmp_unsupported="${_lsmp_unsupported}${_lsmp_unsupported:+,}$_lsmp_partition"
                continue
            fi
            ...
            if [ -d "$_lsmp_source" ]; then
                if luoshu_copy_partition_atomic "$_lsmp_source" "$_lsmp_destination"; then
                    ...
                    [ "$_lsmp_result" -ne 124 ] || LUOSHU_MOUNT_PREFLIGHT_ERROR="元模块同步超过 ${LUOSHU_MOUNT_TIMEOUT} 秒总时限"
                    break
                fi
    ...
    if [ "$_lsmp_failed" -gt 0 ]; then
        luoshu_mount_record failed \
            "${LUOSHU_MOUNT_PREFLIGHT_ERROR:-双目录元模块内容更新失败，已保留旧分区目录}" \
            "$_lsmp_root" "$_lsmp_synced" "$_lsmp_failed" \
            "$_lsmp_partitions" '' "$_lsmp_failed_partitions" "$_lsmp_unsupported"
        return 1
    fi

    _lsmp_detail='已原子写入双目录元模块真实内容镜像，等待重启逐分区验证'
    [ -z "$_lsmp_unsupported" ] || \
        _lsmp_detail="$_lsmp_detail；当前元模块未声明的分区已跳过：$_lsmp_unsupported"
    luoshu_mount_record prepared "$_lsmp_detail" \
        "$_lsmp_root" "$_lsmp_synced" 0 "$_lsmp_partitions" '' '' "$_lsmp_unsupported"
    luoshu_mount_log \
        "engine=$_lsmp_engine backend=$(luoshu_mount_backend "$_lsmp_engine") synced=$_lsmp_synced unsupported=$_lsmp_unsupported"
}
...
luoshu_mount_verify_active() {
    ...
    _lmva_engine=$(luoshu_detect_mount_engine)
    if [ "$_lmva_active" = default ]; then
        luoshu_mount_record verified '系统默认字体无需挂载验证' '' 0 0
        return 0
    fi

    # Direct-source engines are deliberately not probed here. The independent
    # device_font_load_verify.sh checks the real visible font files after boot.
    if luoshu_direct_source_engine "$_lmva_engine"; then
        _lmva_partitions=''
        for _lmva_partition in $(luoshu_used_partitions); do
            _lmva_partitions="${_lmva_partitions}${_lmva_partitions:+,}$_lmva_partition"
        done
        luoshu_mount_record verified \
            '直读挂载引擎由系统实际字体加载验证器确认；兼容层不再用合成探针误判' \
            '' 0 0 "$_lmva_partitions"
        return 0
    fi
    ...
    if [ "$_lmva_failed_count" -eq 0 ]; then
        luoshu_mount_record verified '双目录元模块所有字体负载分区均已从系统路径读取' \
            '' 0 0 "$_lmva_partitions" "$_lmva_verified"
        return 0
    fi
    luoshu_mount_record unverified "部分字体分区未挂载：$_lmva_failed" \
        '' 0 "$_lmva_failed_count" "$_lmva_partitions" "$_lmva_verified" "$_lmva_failed"
    return 1
}
