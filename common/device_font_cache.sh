...
_dfcache_inventory_key() {
    _dfc_module="$(_dfcache_module)"
    _dfc_helper="$_dfc_module/common/font_provenance.sh"
    if [ -f "$_dfc_helper" ]; then
        type luoshu_provenance_inventory_identity >/dev/null 2>&1 || . "$_dfc_helper" >/dev/null 2>&1 || true
...
_dfcache_root_for() {
    ...
    _dfc_source_key="$3"
    _dfc_inventory_key="$4"
    _dfc_id=$(_dfcache_id "$_dfc_font" "$_dfc_template_key" "$_dfc_source_key" "$_dfc_inventory_key") || return 1
...
_dfcache_ready_matches() {
    ...
    [ "$(sed -n 's/^inventoryKey=//p' "$_dfcrm_conf" 2>/dev/null | head -n1)" = "$_dfcrm_inventory_key" ] || return 1
    return 0
}
...
_dfcache_resolve_ready_root() {
    ...
    _dfc_inventory_key="$4"
    _dfc_module="$(_dfcache_module)"
    _dfc_id=$(_dfcache_id "$_dfc_font" "$_dfc_template_key" "$_dfc_source_key" "$_dfc_inventory_key") || return 2
    _dfc_root="$_dfc_module/config/device-font-cache/$_dfc_id"
    _dfcache_ready_matches "$_dfc_root" "$_dfc_font" "$_dfc_template_key" "$_dfc_source_key" "$_dfc_inventory_key" || return 2
    printf '%s\n' "$_dfc_root"
}
...
device_font_cache_lookup() {
    ...
    _dfc_inventory_key=$(_dfcache_inventory_key) || return 2
    _dfcache_resolve_ready_root "$_dfc_font" "$_dfc_template_key" "$_dfc_source_key" "$_dfc_inventory_key" || return 2
    return 0
}
...
device_font_cache_schedule() {
    ...
    _dfc_inventory_key=$(_dfcache_inventory_key) || {
        _dfcache_log '无法安排对齐缓存：本机字体清单身份不可用'
        return 2
    }
    _dfc_id=$(_dfcache_id "$_dfc_font" "$_dfc_template_key" "$_dfc_source_key" "$_dfc_inventory_key") || return 1
...
device_font_cache_activate() {
    ...
    _dfc_source_key=$(_dfcache_source_key) || return 2
    _dfc_inventory_key=$(_dfcache_inventory_key) || return 2
    _dfc_root=$(_dfcache_resolve_ready_root "$_dfc_font" "$_dfc_template_key" "$_dfc_source_key" "$_dfc_inventory_key") || return 2
    _dfc_cache_id=${_dfc_root##*/}
    type _dfpr_install_overlay >/dev/null 2>&1 || return 1
    _dfpr_install_overlay "$_dfc_root/overlay" || return 1
    _dfcache_write_engine_state "$_dfc_font" "$_dfc_template_key" "$_dfc_source_key" "$_dfc_inventory_key" "$_dfc_cache_id" || return 1
...
_dfcache_build_pending_inner() {
    ...
    _dfc_expected_inventory=$(sed -n 's/^inventoryKey=//p' "$_dfc_pending" 2>/dev/null | head -n1)
    _dfc_active=$(head -n1 "$_dfc_module/config/active_font.conf" 2>/dev/null | tr -d '\r\n')
    ...
    _dfc_source_key=$(_dfcache_source_key) || return 2
    _dfc_inventory_key=$(_dfcache_inventory_key) || return 2
    [ "$_dfc_template_key" = "$_dfc_expected_template" ] &&         [ "$_dfc_source_key" = "$_dfc_expected_source" ] &&         [ "$_dfc_inventory_key" = "$_dfc_expected_inventory" ] || {
        _dfcache_log '缓存任务源字体、原厂模板或本机扫描清单已经变化，等待下次明确应用重新安排'
        rm -f "$_dfc_pending" 2>/dev/null || true
        return 2
    }
    _dfc_root="$_dfc_module/config/device-font-cache/$_dfc_cache_id"
    if _dfcache_ready_matches "$_dfc_root" "$_dfc_font" "$_dfc_template_key" "$_dfc_source_key" "$_dfc_inventory_key"; then
