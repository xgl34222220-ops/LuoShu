#!/system/bin/sh
# LuoShu App-only composite engine. Auto mode follows real font metadata.
set +e
...
manual_weight() {
    ...
    [ "$_w" -ge 1 ] 2>/dev/null || _w=1; [ "$_w" -le 1000 ] 2>/dev/null || _w=1000; printf '%s\n' "$_w"
}
weight_role() {
    _n="$1"
...
update_task() {
    _wanted="$1"; _state="$2"; _message="$3"; _percent="$4"; _finished="$5"; _weights="$6"
    ...
    [ -n "$_weights" ] || _weights=$(read_value "$TASK_FILE" weightSet)
    write_task "$_wanted" "$_state" "$_message" "$_cjk" "$_latin" "$_digit" "$_ca" "$_la" "$_da" "$_root" "$_started" "$_finished" "$_percent" "$_weights"
}
...
slot_weights() {
    _family="$1"; _spec="$2"
    if is_auto "$_spec"; then family_weight_numbers "$_family"; else manual_weight "$_spec"; fi
}
resolve_weight_set() {
    _set=""
...
run_instance() {
    ...
    LD_LIBRARY_PATH="$PYROOT/lib:$PYROOT/lib/python3.14/lib-dynload${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
        "$PYBIN" "$INSTANCE_PY" --input "$_src" --output "$_dest" --role "$_role" --weight "$_weight" --axes "$_axes" >"${_dest}.json" 2>"${_dest}.err"
    _rc=$?; [ "$_rc" -eq 0 ] && [ -s "$_dest" ] || return 1
...
prepare_source() {
    ...
    if font_file_is_variable "$_src" || [ "$FONT_CHECK_FORMAT" = TTC ]; then
        ...
        cp -f "$_src" "$_dest" 2>/dev/null || return 1; chmod 0644 "$_dest" 2>/dev/null || true
    fi
    [ -s "$_dest" ] || return 1; printf '%s\n' "$_dest"
}

build_composite() {
    _target="$1"; _cjk="$2"; _latin="$3"; _digit="$4"; _output="$5"
    _tmp="${_output}.raw.ttf"; _report="${_output}.json"; _err="${_output}.err"
    rm -f "$_tmp" "$_output" "$_report" "$_err" 2>/dev/null || true
    ...
    _rc=$?; [ "$_rc" -eq 0 ] && [ -s "$_tmp" ] || { cat "$_err" >&2 2>/dev/null; return 1; }
    PYTHONHOME="$PYROOT" PYTHONPATH="$PYROOT/lib/python3.14:$PYROOT/lib/python3.14/site-packages" \
    LD_LIBRARY_PATH="$PYROOT/lib:$PYROOT/lib/python3.14/lib-dynload${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
        "$PYBIN" "$REWRITE_PY" "$_tmp" "$_output" --family "$PRIVATE_FAMILY" --weight "$_target" >>"$_report" 2>>"$_err" || return 1
...
write_mix_config() {
    ...
    {
        printf 'cjk=%s\nlatin=%s\ndigit=%s\n' "$_cjk" "$_latin" "$_digit"
        printf 'cjkWeight=%s\nlatinWeight=%s\ndigitWeight=%s\n' "$(manual_weight "${_ca/auto/wght=400}")" "$(manual_weight "${_la/auto/wght=400}")" "$(manual_weight "${_da/auto/wght=400}")"
        printf 'cjkAxes=%s\nlatinAxes=%s\ndigitAxes=%s\n' "$_ca" "$_la" "$_da"
        printf 'appOnly=true\nmultiWeight=true\nmultiWeightSet=%s\n' "$_set"
    ...
    printf 'cjkAuto=%s\nlatinAuto=%s\ndigitAuto=%s\n' "$_cauto" "$_lauto" "$_dauto" >"$APP_MODE_CONF.tmp.$$" && mv -f "$APP_MODE_CONF.tmp.$$" "$APP_MODE_CONF"
    cp -f "$MIX_CONF" "$AXIS_CONF" 2>/dev/null || true; printf 'mix\n' >"$ACTIVE_CONF"
    chmod 0644 "$MIX_CONF" "$AXIS_CONF" "$ACTIVE_CONF" "$APP_MODE_CONF" 2>/dev/null || true
}
...
worker() {
    ...
    update_task "$_wanted" running "已识别真实字重：$_set" 3 '' "$_set"
    ...
    IFS="$_oldifs"
    _default=$(family_file_for_weight "$PRIVATE_FAMILY" 400)
    [ -s "$_default" ] || { update_task "$_wanted" failed '没有生成可用默认字重' 100 "$(date +%s)" "$_set"; exit 1; }
    ...
    write_mix_config "$_cjk" "$_latin" "$_digit" "$_ca" "$_la" "$_da" "$_set" || { update_task "$_wanted" failed '字体已生成，但组合状态提交失败' 100 "$(date +%s)" "$_set"; exit 1; }
...
json_weight_array() { printf '%s' "$1" | sed 's/^/[ /; s/,/, /g; s/$/ ]/'; }
status_json() {
    ...
    printf '{"status":"ok","data":{"task":"%s","state":"%s","message":"%s","cjk":"%s","latin":"%s","digit":"%s","cjkWeight":%s,"latinWeight":%s,"digitWeight":%s,"cjkAuto":%s,"latinAuto":%s,"digitAuto":%s,"multiWeightSet":%s,"started":%s,"finished":%s,"progress":{"message":"%s","percent":%s}}}\n' \
        "$(json_escape "$_task")" "$(json_escape "$_state")" "$(json_escape "$_message")" "$(json_escape "$(read_value "$TASK_FILE" cjk)")" "$(json_escape "$(read_value "$TASK_FILE" latin)")" "$(json_escape "$(read_value "$TASK_FILE" digit)")" \
        "$(manual_weight "${_ca/auto/wght=400}")" "$(manual_weight "${_la/auto/wght=400}")" "$(manual_weight "${_da/auto/wght=400}")" "$_cauto" "$_lauto" "$_dauto" "$(json_weight_array "$_set")" \
        "${_started:-$(read_value "$TASK_FILE" started)}" "${_finished:-$(read_value "$TASK_FILE" finished)}" "$(json_escape "$_message")" "${_percent:-$(read_value "$TASK_FILE" percent)}"
}
config_json() {
    ...
    printf '{"status":"ok","data":{"enabled":%s,"cjk":"%s","latin":"%s","digit":"%s","cjkWeight":%s,"latinWeight":%s,"digitWeight":%s,"cjkAuto":%s,"latinAuto":%s,"digitAuto":%s,"multiWeightSet":%s}}\n' \
        "$_enabled" "$(json_escape "$_cjk")" "$(json_escape "$_latin")" "$(json_escape "$_digit")" "$(manual_weight "${_ca/auto/wght=400}")" "$(manual_weight "${_la/auto/wght=400}")" "$(manual_weight "${_da/auto/wght=400}")" \
        "$_cauto" "$_lauto" "$_dauto" "$(json_weight_array "$_set")"
}
start_mix() {
    ...
    [ -n "$_ca" ] || _ca=auto; [ -n "$_la" ] || _la=auto; [ -n "$_da" ] || _da=auto
    _request="appmw-$(date +%s)-$$"; _root="$CACHE_ROOT/$_request"; mkdir -p "$_root/fonts" "$_root/sources" "$CONFIG_DIR" "$MODDIR/logs" 2>/dev/null || { printf '{"status":"error","message":"无法创建任务目录"}\n'; return; }
    write_task "$_request" queued '正在识别字体自身字重' "$_cjk" "$_latin" "$_digit" "$_ca" "$_la" "$_da" "$_root" "$(date +%s)" '' 1 ''
    ( MODDIR="$MODDIR" sh "$0" worker "$_request" ) </dev/null >>"$LOG_FILE" 2>&1 &
    printf '%s\n' "$!" >"$WORKER_PID" 2>/dev/null || true; printf '{"status":"ok","data":{"task":"%s"}}\n' "$(json_escape "$_request")"
}
recover_task() {
    if [ -s "$TASK_FILE" ]; then _state=$(read_value "$TASK_FILE" state); _task=$(read_value "$TASK_FILE" task); case "$_state" in queued|running) update_task "$_task" failed '上次任务被开机恢复中止' 100 "$(date +%s)" '' ;; esac; fi
