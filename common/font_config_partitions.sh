...
_luoshu_font_config_partition_rows() {
    _lfcp_payload="$(_luoshu_font_config_payload_root)"
    printf 'system|%s|%s/system\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_SYSTEM_ROOT:-}" "${LUOSHU_SYSTEM_ETC_ROOT:-}" /system/etc)" "$_lfcp_payload"
    printf 'system_ext|%s|%s/system_ext\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_SYSTEM_EXT_ROOT:-}" "${LUOSHU_SYSTEM_EXT_ETC_ROOT:-}" /system_ext/etc)" "$_lfcp_payload"
    printf 'product|%s|%s/product\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_PRODUCT_ROOT:-}" "${LUOSHU_PRODUCT_ETC_ROOT:-}" /product/etc)" "$_lfcp_payload"
    printf 'vendor|%s|%s/vendor\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_VENDOR_ROOT:-}" "${LUOSHU_VENDOR_ETC_ROOT:-}" /vendor/etc)" "$_lfcp_payload"
    printf 'odm|%s|%s/odm\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_ODM_ROOT:-}" "${LUOSHU_ODM_ETC_ROOT:-}" /odm/etc)" "$_lfcp_payload"
    printf 'oem|%s|%s/oem\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_OEM_ROOT:-}" "${LUOSHU_OEM_ETC_ROOT:-}" /oem/etc)" "$_lfcp_payload"
    printf 'my_product|%s|%s/my_product\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_MY_PRODUCT_ROOT:-}" "${LUOSHU_MY_PRODUCT_ETC_ROOT:-}" /my_product/etc)" "$_lfcp_payload"
    printf 'my_engineering|%s|%s/my_engineering\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_MY_ENGINEERING_ROOT:-}" "${LUOSHU_MY_ENGINEERING_ETC_ROOT:-}" /my_engineering/etc)" "$_lfcp_payload"
    printf 'my_company|%s|%s/my_company\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_MY_COMPANY_ROOT:-}" "${LUOSHU_MY_COMPANY_ETC_ROOT:-}" /my_company/etc)" "$_lfcp_payload"
    printf 'my_preload|%s|%s/my_preload\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_MY_PRELOAD_ROOT:-}" "${LUOSHU_MY_PRELOAD_ETC_ROOT:-}" /my_preload/etc)" "$_lfcp_payload"
    printf 'my_region|%s|%s/my_region\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_MY_REGION_ROOT:-}" "${LUOSHU_MY_REGION_ETC_ROOT:-}" /my_region/etc)" "$_lfcp_payload"
    printf 'my_stock|%s|%s/my_stock\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_MY_STOCK_ROOT:-}" "${LUOSHU_MY_STOCK_ETC_ROOT:-}" /my_stock/etc)" "$_lfcp_payload"
    printf 'oplus_product|%s|%s/oplus_product\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_OPLUS_PRODUCT_ROOT:-}" "${LUOSHU_OPLUS_PRODUCT_ETC_ROOT:-}" /oplus_product/etc)" "$_lfcp_payload"
    printf 'oplus_engineering|%s|%s/oplus_engineering\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_OPLUS_ENGINEERING_ROOT:-}" "${LUOSHU_OPLUS_ENGINEERING_ETC_ROOT:-}" /oplus_engineering/etc)" "$_lfcp_payload"
    printf 'oplus_version|%s|%s/oplus_version\n' "$(_luoshu_font_config_resolve_etc "${LUOSHU_OPLUS_VERSION_ROOT:-}" "${LUOSHU_OPLUS_VERSION_ETC_ROOT:-}" /oplus_version/etc)" "$_lfcp_payload"
