...
export PYTHONPATH="$MODDIR/common:$RUNTIME/lib/python3.14:$RUNTIME/lib/python3.14/site-packages"
...
if [ "${1:-}" = "--self-test" ]; then
    exec "$PYBIN" -c 'import fontTools; import composite_aligned; print("ok")'
fi
exec "$PYBIN" "$MODDIR/common/composite_aligned.py" "$@"
