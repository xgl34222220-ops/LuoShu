...
# module.prop 区分用户选择、待验证与有字节证据的生效状态。
set +e
...
if [ "$ACTIVE" != default ]; then
    ...
    if [ -f "$MODDIR/common/device_font_load_verify.sh" ]; then
        MODDIR="$MODDIR" MODULE_DIR="$MODDIR" \
            sh "$MODDIR/common/device_font_load_verify.sh" status >/dev/null 2>&1
        VERIFY_RC=$?
    fi
    ...
    MOUNT_BACKEND=$(sed -n 's/^backend=//p' "$MODDIR/config/self-mount.conf" 2>/dev/null | head -n1)

    if [ -f "$MODDIR/config/text_reboot_required.conf" ]; then
        EFFECTIVE_DISPLAY="已配置：$DISPLAY（等待重启）"
    elif [ "$MOUNT_STATE:$MOUNT_BACKEND" = failed:rollback ]; then
        EFFECTIVE_DISPLAY="系统默认字体（$DISPLAY 未生效）"
    elif [ "$MOUNT_STATE" = failed ] || [ "$VERIFY_STATE" = failed ]; then
        EFFECTIVE_DISPLAY="已选择：$DISPLAY（验证异常）"
    elif [ "$VERIFY_STATE" = verified ] && [ "$VERIFY_RC" -eq 0 ] && [ "$VERIFY_ACTIVE" = "$ACTIVE" ]; then
        EFFECTIVE_DISPLAY="$DISPLAY"
    else
        EFFECTIVE_DISPLAY="已选择：$DISPLAY（待验证）"
    fi
fi
