...
device_font_payload_build_install() { : > "$TMP/cache.lookup"; return 2; }
device_font_cache_schedule() { : > "$TMP/cache.schedule"; return 0; }
...
test "$LUOSHU_DEVICE_PAYLOAD_RESULT" = 'slot+xml'
test "$(wc -l < "$TMP/generator.calls" | tr -d '[:space:]')" -eq 1
test ! -e "$TMP/cache.lookup"
