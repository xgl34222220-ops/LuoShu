...
grep -q 'safe_switch_cache_dir_kb' "$ROOT/common/legacy_v14_4/font_switch_safe.sh"
...
grep -q 'coverageComplete=%s' "$ROOT/common/legacy_v14_4/font_switch_safe.sh"
grep -q 'SAFE_CACHE_COVERAGE_COMPLETE' "$ROOT/common/legacy_v14_4/font_switch_safe.sh"
grep -q "progress 84 '已复用完整字体槽位缓存，跳过重复补齐'" "$ROOT/common/legacy_v14_4/font_switch_safe.sh"
grep -q 'while \[ "$_wfpc_steps" -lt 4 \]' "$ROOT/common/legacy_v14_4/font_switch_safe.sh"
...
grep -q 'PowerManager.PARTIAL_WAKE_LOCK' "$ROOT/android-app/app/src/main/java/io/github/xgl34222220/luoshu/LuoShuViewModel.kt"
grep -q 'reconcileTaskAfterResume' "$ROOT/android-app/app/src/main/java/io/github/xgl34222220/luoshu/LuoShuViewModel.kt"
grep -q 'android.permission.WAKE_LOCK' "$ROOT/android-app/app/src/main/AndroidManifest.xml"
! grep -q 'bridge)} validate' "$ROOT/android-app/app/src/main/java/io/github/xgl34222220/luoshu/LuoShuViewModel.kt"
