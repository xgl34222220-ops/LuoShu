...
grep -q '^font=mix# did not. The preserved stage metadata is sufficient to finish without rebuild.
...
grep -q '^requestId=request-a# did not. The preserved stage metadata is sufficient to finish without rebuild.
rm -f "$MODULE/config/font-payload-next.conf"
cat > "$MODULE/config/mix-stage-next.conf" <<'EOF_RECOVER'
requestId=request-a
cjk=CjkA
