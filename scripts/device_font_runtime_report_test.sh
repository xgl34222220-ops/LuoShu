...
printf '{"schema":"device-font-inventory-v1","buildKey":"fixture","slots":{}}' > "$MOD/config/device_font_inventory.json"
printf '{"schema":"device-font-candidates-v1","paths":[{"path":"/system/fonts/NotoColorEmoji.ttf","partition":"system","slotName":"NotoColorEmoji.ttf","candidate":false,"reason":"specialized-name"}]}' > "$MOD/config/device_font_candidates.json"
...
ok test -s "$REPORT/device-font-inventory.json"
ok test -s "$REPORT/device-font-candidates.json"
ok grep -q 'NotoColorEmoji.ttf' "$REPORT/device-font-candidates.json"
ok test -s "$REPORT/device-font-roots.conf"
ok grep -qx 'product|vivo/fonts|product-nested-fixture' "$REPORT/device-font-roots.conf"
