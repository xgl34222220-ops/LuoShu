...
# v14.1：切换/组合完成时同步；APatch post-mount 与 late_start service 补同步；post-fs-data 保持轻量。
...
cp "$ROOT/post-mount.sh" "$STAGE/post-mount.sh"
...
grep -q 'luoshu_sync_mount_payload' "$STAGE/common/font_switch_v141.sh"
grep -q 'luoshu_sync_mount_payload' "$STAGE/common/font_mix.sh"
grep -q 'luoshu_sync_mount_payload' "$STAGE/post-mount.sh"
...
echo 'LuoShu v14.1 mount compatibility checks passed.'
