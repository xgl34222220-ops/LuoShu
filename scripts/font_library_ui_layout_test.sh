...
LAYOUT_TOKENS="$ROOT/android-app/app/src/main/java/io/github/xgl34222220/luoshu/ui/theme/LuoShuLayoutTokens.kt"
...
grep -q 'HomeWeightSlider' "$HOME_COMPACT"
grep -q 'HapticFeedbackType.TextHandleMove' "$HOME_COMPACT"
grep -q '不改字体文件 · 随时可恢复' "$HOME_COMPACT"
...
grep -q 'row.label.contains("ID", ignoreCase = true)' "$DETAILS"
grep -q 'softWrap = false' "$DETAILS"
...
grep -q 'targetValue = if (childLayerActive) .95f else 1f' "$STUDIO_ROUTE"
grep -q 'dampingRatio = .85f' "$STUDIO_ROUTE"
grep -q 'targetValue = if (childLayerActive) .95f else 1f' "$ROUTE"
grep -q 'dampingRatio = .85f' "$ROUTE"
grep -q 'ModalBottomSheet' "$STUDIO_PREVIEW"
grep -q 'scrimColor = Color.Black.copy(alpha = .20f)' "$STUDIO_PREVIEW"
! grep -q 'DialogProperties' "$STUDIO_PREVIEW"
...
grep -q '重新检核' "$SETTINGS"
grep -q 'LuoShuLoadingSkeleton' "$SETTINGS"
...
grep -q 'pageBackground = Color(0xFFF8FAFC)' "$THEME"
grep -q 'val FloatingDockSafeBottom = 104.dp' "$LAYOUT_TOKENS"
grep -q 'val SecondaryBlueSurface = Color(0xFFF0F4FF)' "$LAYOUT_TOKENS"
