...
STUDIO_AXIS="$ROOT/android-app/app/src/main/java/io/github/xgl34222220/luoshu/ui/studio/StudioAxisControls.kt"
STUDIO_SLOT_PREVIEW="$ROOT/android-app/app/src/main/java/io/github/xgl34222220/luoshu/ui/studio/StudioSlotPreview.kt"
STUDIO_PREVIEW="$ROOT/android-app/app/src/main/java/io/github/xgl34222220/luoshu/ui/studio/StudioCompositePreviewCompact.kt"
METADATA_INSPECTOR="$ROOT/android-app/app/src/main/java/io/github/xgl34222220/luoshu/FontMetadataInspector.kt"
...
grep -q 'InteractiveAxisSlider' "$STUDIO_AXIS"
grep -q 'height(72.dp)' "$STUDIO_AXIS"
grep -q 'listOf(400f, 700f)' "$STUDIO_AXIS"
grep -q 'steps = 0' "$STUDIO_AXIS"
grep -q 'fontFeatureSettings = "tnum"' "$STUDIO_AXIS"
grep -q 'StudioSlotPreview' "$STUDIO_MIUIX"
grep -q 'StudioSlotPreview' "$STUDIO_MATERIAL"
grep -q 'Color(0xFFF0F4F9)' "$STUDIO_SLOT_PREVIEW"
grep -q 'Icons.Rounded.DarkMode' "$STUDIO_SLOT_PREVIEW"
grep -q 'Brush.horizontalGradient' "$STUDIO_PREVIEW"
grep -q 'scrollState.maxValue' "$STUDIO_PREVIEW"
...
grep -q 'sheetState.hide()' "$STUDIO_TOOLS"
grep -q 'openChild(onPreview)' "$STUDIO_TOOLS"
! grep -q 'parentAlpha' "$STUDIO_TOOLS"
! grep -q 'childLayerActive' "$STUDIO_TOOLS"
...
grep -q 'background(solidPageColor)' "$SHELL"
! grep -q 'alpha = .94f + (.06f \* pageEnter.value)' "$SHELL"
...
grep -q 'key(section?.name ?: "settings-home")' "$SETTINGS"
grep -q 'background(pageBackground)' "$SETTINGS"
! grep -q 'targetAlpha = .52f' "$SETTINGS"
...
grep -q 'WeightStepButton' "$HOME_COMPACT"
grep -q 'steps = 0' "$HOME_COMPACT"
grep -q '个性化字形配置' "$HOME_COMPACT"
grep -q 'FontAvatar(font)' "$COMPACT"
grep -q 'fontAvatarColor' "$COMPACT"
grep -q 'MetadataMetricCard' "$METADATA_INSPECTOR"
grep -q 'MetadataHashCard' "$METADATA_INSPECTOR"
! grep -q 'Text(.*result.text' "$METADATA_INSPECTOR"
