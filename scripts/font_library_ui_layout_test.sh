...
grep -q 'fontSize = 30.sp' "$STUDIO_MIUIX"
grep -q 'fontSize = 30.sp' "$STUDIO_MATERIAL"
grep -q 'horizontalArrangement = Arrangement.spacedBy(6.dp)' "$STUDIO_MIUIX"
...
grep -q 'val HeaderContainer = 36.dp' "$ICON_SYSTEM"
grep -q 'val HeaderGlyph = 18.dp' "$ICON_SYSTEM"
...
grep -q 'RoundedCornerShape(19.dp)' "$SETTINGS"
