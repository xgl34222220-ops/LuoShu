...
if ! LUOSHU_HOST_TEST_ALLOW_NO_SCAN=1 MODPATH="$NEW" LUOSHU_OLD_MOD="$OLD" sh -x "$ROOT/customize.sh" >"$TMP/direct.log" 2>&1; then
...
if ! LUOSHU_HOST_TEST_ALLOW_NO_SCAN=1 MODPATH="$NEW" LUOSHU_OLD_MOD="$OLD" sh -x -c '
    . "$1" >"$3" 2>&1
    printf "continued\n" > "$2"
