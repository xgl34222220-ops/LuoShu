...
mkdir -p "$CURRENT/config" "$CURRENT/system/fonts" \
    "$CURRENT/cache/full-composite-v10" "$CURRENT/cache/auto-multiweight-mix/composites-v7" \
    "$CURRENT/cache/auto-multiweight-mix/prepared-v7" "$CURRENT_NEW/config"
printf 'id=LuoShu\nversion=current\n' >"$CURRENT/module.prop"
printf 'Qsal\n' >"$CURRENT/config/active_font.conf"
printf 'schema=%s\nfont=Qsal\n' "$SCHEMA" >"$CURRENT/config/font-payload-schema.conf"
printf 'payload\n' >"$CURRENT/system/fonts/Qsal-Regular.ttf"
printf 'v10\n' >"$CURRENT/cache/full-composite-v10/current.font"
printf 'v7\n' >"$CURRENT/cache/auto-multiweight-mix/composites-v7/current.font"
printf 'prepared\n' >"$CURRENT/cache/auto-multiweight-mix/prepared-v7/current.font"
luoshu_migrate_active_install "$CURRENT" "$CURRENT_NEW"
test "$LUOSHU_UPDATE_REBUILD_REQUIRED" = false
test -f "$CURRENT_NEW/cache/full-composite-v10/current.font"
test -f "$CURRENT_NEW/cache/auto-multiweight-mix/composites-v7/current.font"
test -f "$CURRENT_NEW/cache/auto-multiweight-mix/prepared-v7/current.font"
