...
make_install() {
    ...
    printf 'id=LuoShu\nversion=v4.2.1\nversionCode=40201\n' > "$1/module.prop"
    for _fixture_builder in hyperos_physical_policy.py hyperos_metrics_batch.py coloros_metrics_batch.py; do
        printf '# builder policy before update\n' > "$1/common/$_fixture_builder"
    done
}
...
for _changed_builder in hyperos_physical_policy.py hyperos_metrics_batch.py coloros_metrics_batch.py; do
    rm -rf "$NEW"
    make_install "$NEW"
    printf '# narrowed routing or corrected physical metrics\n' > "$NEW/common/$_changed_builder"
...
FINAL="$TMP/final"
make_install "$FINAL"
cp "$NEXT/common/"*.py "$FINAL/common/"
