...
grep -q '正在完成本机安全字体槽位缓存' "$ROOT/common/legacy_v14_4/font_switch_safe.sh"
grep -q '已复用完整字体槽位缓存，跳过重复补齐' "$ROOT/common/legacy_v14_4/font_switch_safe.sh"
grep -q 'coverageComplete=%s' "$ROOT/common/legacy_v14_4/font_switch_safe.sh"
grep -q '正在补齐剩余本机安全字体槽位' "$ROOT/common/legacy_v14_4/mix_router.sh"
grep -q 'mix_post_cache_restore' "$ROOT/common/legacy_v14_4/mix_router.sh"
grep -q '单槽度量归一化失败，已回退真实字体源' "$ROOT/common/coverage_payload_remediate.sh"
grep -q 'preserved-normalization-failure' "$ROOT/common/coverage_payload_remediate.sh"
grep -q '已保留 ROM 原字体' "$ROOT/common/coverage_payload_remediate.sh"
grep -q 'run_mix_coverage_helper' "$ROOT/common/legacy_v14_4/mix_router.sh"
grep -q '.mix-coverage-result.' "$ROOT/common/legacy_v14_4/mix_router.sh"
