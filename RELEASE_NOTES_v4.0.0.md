...
本次直接覆盖此前不可用的 v4.0.0，不另起版本号。重点修复真机“任务显示成功但完整重启后字体没有变化”、复合字体卡在 34%、英文数字仍沿用上一次选择、HyperOS 不同 App 字体上下偏移、原厂字体手动扫描不可用，以及界面误报 `read interrupted by close()` 的问题。

## 复合字体任务不再卡 34%

- App 查询组合配置和任务状态时直接读取原子状态文件，不再反复初始化兼容运行时；消除轮询超时导致的 34% 长时间停顿与假失败。
- 复合生成、暂存和下一启动提交使用同一个请求 ID，并校验中文、英文、数字源字体哈希及最终成品哈希；本次任务不会接受上一次遗留成品。
- 后台生成完成后独立提交 `.luoshu-payload-next`，只有真实下一启动负载和选择状态同时写入后才报告 100%；App 被系统关闭或读取中断不影响提交。
- `read interrupted by close() on another thread` 作为瞬时通信中断处理，不再直接显示为“字体组合失败”。
...
- 不同物理字体槽分别读取并保持对应原厂度量合约，相同合约才复用缓存；状态栏、酷安、QQ 等各自使用的 Latin/数字槽不再被一套全局行框强行覆盖。
...
- 扫描增加跨进程序列锁；安装、开机预热和手动扫描不会再同时读取同一字体树并互相关闭连接。
- KernelSU/SukiSU 在洛书 self-mount 接管 `/system/fonts` 前完成原厂清单冻结；已验证的早期启动视图可直接用于扫描，不再错误提示 lower/mirror 不可用。

## 版本与替换说明

- 模块继续使用 v4.0.0（40000）。
- App 继续使用 v4.0.0（4000001）。
- GitHub 上此前 v4.0.0 的 Release、tag 与 APK/ZIP 资产属于已确认不可用的旧构建；本次发布流程会删除旧 Release/tag 并用修复后的同版本产物重新创建，下载入口仍保持 v4.0.0。
- 覆盖安装后请重新应用一次需要使用的单字体或复合字体，等待任务完成到 100%，然后执行一次完整重启。
