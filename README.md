...
- v13.6 Beta6 默认使用 Direct Bind，未选择自定义 Emoji 时安装包和已安装模块都不保留 `system/` 分区负载，因此无需 Hybrid Mount 建立字体 staging。HyperOS 会关闭 GMS FontsProvider 的动态字体覆盖，使 Play 回退到已替换的系统字体；ColorOS 保持动态字体桥接。
