...
- 修复恢复系统字体后首页仍提示“设备字体尚未验证”，系统字体现在明确显示无需验证。
- 可信状态明确区分系统字体、设备负载已验证、兼容字体映射、等待验证与异常，不再把所有状态混为等待重启。
- 可信状态详情增加当前字体类型与真实验证原因，避免用户反复应用字体或无效重启。
- 开机加载验证增加延迟与重试，并在 `late_start service` 完成模板、索引和负载维护后再次执行最终检查。
- MIUIx 任务中心诊断与刷新按钮改为同一布局行、相同 56 dp 尺寸和共同垂直中线，彻底移除固定顶部偏移方案。
- 正式版本为 `v2.2.4 / 20204`，正式 App `versionCode` 为 `2020401`。

## [v2.2.3] - 2026-07-25

- 完整重启后自动执行设备字体加载验证，并等待 Android 启动完成和字体事务空闲，修复首页长期显示“设备字体尚未验证”。
- 对 OEM 系统隐藏或裁剪 `cmd font dump` 明细的情况，在全部系统可见挂载字体与洛书负载 SHA-256 一致时，使用明确标注的“挂载已验证”状态。
- 每次字体切换成功后重新进入等待重启验证，避免错误沿用上一套字体的验证结论。
- App 应用字体前只做快速文件与格式预检；完整字形、覆盖与安全验证继续在后台事务中执行。
- 单字体切换改为独立 Root Worker，关闭 App、切到后台或 App 被回收后任务仍可继续。
- 安全超时由 45 秒提升至 110 秒，适配大型 CJK、TTC/OTC 字体和较慢存储设备；真正超时仍终止并回滚上一套字体。
- 修复 MIUIx 任务中心右上角诊断与刷新按钮视觉中线不一致，并优化首页未验证提示。
- 正式版本为 `v2.2.3 / 20203`，正式 App `versionCode` 为 `2020301`。
