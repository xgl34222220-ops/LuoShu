...
- v2.0.3-rc1：候选版版本号提升至 `v2.0.3-rc1 / 20003`，正式 App `versionCode` 为 `2000301`。
- 统一中文、英文与数字的真实轮廓、基线和垂直偏移，继续修正输入框、标题、桌面名称与混排页面的视觉错位。
- 补齐 ColorOS Google Play 搜索框等直连字体槽，并强化 system、product、system_ext、my_product 等真实分区映射。
- 九档字重覆盖继续使用真实静态字重或可变字体实例，不以仅修改元数据冒充真实轮廓。
- 减少字体校验、生成、复制和负载收尾中的重复工作，保持事务校验与失败回滚完整。
- 修正当前字体状态误显示为系统默认、历史任务残留常驻和无效日志噪声。
- 完善 Magic Mount RC/RS 与多 OEM 分区适配，候选包需通过完整 PR CI 后再进入预发行。
