...
- 首页增加设备可信状态、开机加载证据、脱敏诊断摘要和真机验收向导，不把兼容或未知状态伪装成验证通过。
- 字体工坊增加当前系统与候选方案 A/B 组合预览、四套快速字重方案、版本化 JSON 导入导出和 Unicode 字形浏览器。
- 字体库增加 Family 分组、收藏、标签、重复与命名冲突提示，以及批量整理能力。
- 增加 SAF 字体目录监视、配置备份恢复和发布前迁移检查；目录删除只提示，不自动删除字体库内容。
- 增加真实字体文件 ZIP 归档，导出用户字体库中的 TTF、OTF、TTC 文件及逐文件 SHA-256 `manifest.json`，不导出设备原厂字体或私人路径。
- 增加匿名真机测试矩阵，仅记录 Android API、Root 管理器、挂载引擎、加载模式和验收结果，不记录型号、设备指纹、序列号、字体名称或文件路径。
- 增加预发行就绪报告工作流，可检查版本号、versionCode、版本说明、关键脚本和候选成品，并在正式准备时启用阻断模式。
- 修复候选工作流版本硬编码与 Android 编译错误摘要缺失，新增字体归档、设备矩阵、方案、字形分页、迁移和可信状态回归测试。
- 候选版本提升为 `v2.2.2-alpha1 / 20202`，App `versionCode` 为 `2020201`；本候选不修改字体生成、挂载、度量、事务和回滚核心逻辑。
